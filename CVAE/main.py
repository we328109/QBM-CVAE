...
class TrajDataset(Dataset):
    def __init__(self, file):
        ...
        self.features = np.array(pd.read_csv(file).iloc[:,11:].fillna(0).values, dtype=float)
        self.a = np.array(pd.read_csv(file).iloc[:,5].fillna(0).values, dtype=float)
        self.b = np.array(pd.read_csv(file).iloc[:,6].fillna(0).values, dtype=float)
        self.c = np.array(pd.read_csv(file).iloc[:,7].fillna(0).values, dtype=float)
        self.alpha = np.array(pd.read_csv(file).iloc[:,8].fillna(0).values, dtype=float)/180.0 * 3.1415926
        self.beta = np.array(pd.read_csv(file).iloc[:,9].fillna(0).values, dtype=float)/180.0 * 3.1415926
        self.gamma = np.array(pd.read_csv(file).iloc[:,10].fillna(0).values, dtype=float)/180.0 * 3.1415926
        self.crystals = np.array(pd.read_csv(file).iloc[:,1].fillna(0).values, dtype=float)
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch == 100:
            for pg in optimizer.param_groups:
                pg["weight_decay"] = args.weight_decay  # 1e-4
            print(f"Enable weight_decay={args.weight_decay} from epoch {epoch}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_dataset_dir",
        default="./data/train_phases.csv",
        type=str,
        help="Directory where dataset is stored.",
    )
    parser.add_argument(
        "--test_dataset_dir",
        default="./data/test_phases.csv",
        type=str,
        help="Directory where dataset is stored.",
    )
    parser.add_argument(
