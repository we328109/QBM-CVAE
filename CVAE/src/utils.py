...
def r2_cal(inp, tar):
    ...
    for i, t in zip(inp, tar):
        # i/t 在你的代码里是“list of np.array（每个batch一个）”
        i = np.concatenate([np.asarray(x).reshape(-1) for x in i], axis=0)
        t = np.concatenate([np.asarray(x).reshape(-1) for x in t], axis=0)
        R2_list.append(r2_score(i, t))
    return R2_list
'''
def r2_cal(inp, tar):
    R2_list = list()
